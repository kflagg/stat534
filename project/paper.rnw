...
<<polyfit1, cache = TRUE, warning = FALSE, fig.width = 6, fig.height = 8, fig.pos = 'p', fig.cap = ''>>=
...
ex_p10 <- predict(ex_fit10, window = site_window)
ex_p11 <- predict(ex_fit11, window = site_window)
ex_p12 <- predict(ex_fit12, window = site_window)
...
plot(43560 * ex_p10, main = '10th Degree')
plot(43560 * ex_p11, main = '11th Degree')
plot(43560 * ex_p12, main = '12th Degree')
plot(43560 * density(ex_ppp_full) * area(ex_ppp_full) / ex_ppp_full$n,
     main = 'Kernel Density')
@

<<polyse1, fig.width = 6, fig.height = 8, fig.pos = 'p', fig.cap = ''>>=
par(mfrow = c(4, 3), mar = c(1, 0, 2, 3), las = 2, cex = 1)
plot(log(43560 * ex_p2$se), main = '2nd Degree\nlog(SE)', zlim = c(6.5, 10.5))
plot(log(43560 * ex_p3$se), main = '3rd Degree\nlog(SE)', zlim = c(6.5, 10.5))
plot(log(43560 * ex_p4$se), main = '4th Degree\nlog(SE)', zlim = c(6.5, 10.5))
plot(ex_p2$se / ex_p2$estimate, main = '2nd Degree\nRSE', zlim = c(0, 2))
plot(ex_p3$se / ex_p3$estimate, main = '3rd Degree\nRSE', zlim = c(0, 2))
