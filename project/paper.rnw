...
\newcommand\duedate{April 2, 2017}
...
<<polyfit1, cache = TRUE, dependson = 'sim2000obs', warning = FALSE>>=
...
ex_kde <- density(ex_ppp_full)
@

Figure~\ref{fig:polyplot1} shows the predicted intensity surfaces for the
polynomial models, along with a kernel density estimate for comparison. The
2nd-order polynomial surface is too simple to capture the bimodal surface,

The kernel density estimate is computed using the \texttt{spatstat}'s default
...
along the transects. As it turns out, the default bandwith is large enough to
...
<<polyplot1, fig.width = 6, fig.height = 8, fig.pos = 'p', fig.cap = 'Predicted intensity surfaces for eleven polynomial models, and a kernel density estimate for comparision.'>>=
...
plot(43560 * ex_p2$estimate, zlim = c(0, 500), main = '2nd Degree')
plot(43560 * ex_p3$estimate, zlim = c(0, 500), main = '3rd Degree')
plot(43560 * ex_p4$estimate, zlim = c(0, 500), main = '4th Degree')
plot(43560 * ex_p5$estimate, zlim = c(0, 500), main = '5th Degree')
plot(43560 * ex_p6$estimate, zlim = c(0, 500), main = '6th Degree')
plot(43560 * ex_p7$estimate, zlim = c(0, 500), main = '7th Degree')
plot(43560 * ex_p8$estimate, zlim = c(0, 500), main = '8th Degree')
plot(43560 * ex_p9$estimate, zlim = c(0, 500), main = '9th Degree')
plot(43560 * ex_p10$estimate, zlim = c(0, 500), main = '10th Degree')
plot(43560 * ex_p11$estimate, zlim = c(0, 500), main = '11th Degree')
plot(43560 * ex_p12$estimate, zlim = c(0, 500), main = '12th Degree')
plot(43560 * ex_kde * area(ex_ppp_full) / area(ex_ppp), zlim = c(0, 500),
     main = 'Kernel Density')
@

<<polyse1, fig.width = 6, fig.height = 8, fig.pos = 'p', fig.cap = 'Log standard errors and relative standard errors for six of the polynomial models.'>>=
par(mfrow = c(4, 3), mar = c(1, 0, 2, 3), las = 2, cex = 1)
plot(log(43560 * ex_p2$se), main = '2nd Degree\nlog(SE)')
plot(log(43560 * ex_p4$se), main = '4th Degree\nlog(SE)')
plot(log(43560 * ex_p6$se), main = '6th Degree\nlog(SE)')
plot(ex_p2$se / ex_p2$estimate, main = '2nd Degree\nRSE')
plot(ex_p4$se / ex_p4$estimate, main = '4th Degree\nRSE')
plot(ex_p6$se / ex_p6$estimate, main = '6th Degree\nRSE')
...
plot(ex_p8$se / ex_p8$estimate, main = '8th Degree\nRSE')
plot(ex_p10$se / ex_p10$estimate, main = '10th Degree\nRSE')
plot(ex_p12$se / ex_p12$estimate, main = '12th Degree\nRSE')
@
