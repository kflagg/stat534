...
\section{Simple UXO Site}

<<uxoexampledata>>=
# Path to my writing project root directory.
vspuxo <- file.path('..', '..', 'vspuxo')

# Load my functions for working with UXO data.
source(file.path(vspuxo, 'rfns', 'data_functions.r'))
source(file.path(vspuxo, 'rfns', 'sampling_functions.r'))
source(file.path(vspuxo, 'rfns', 'spatial_functions.r'))
...
# The gauss.elliptic function from spatial_functions.r is a bivariate normal
# intensity parameterized in terms of the center, major and minor axis scales,
# and angle of the major axis.
...
plot(setminus.owin(owin(c(1556870, 1564495),
                        c(535421, 541130)),
...
ex_anom <- read.csv(file.path(vspuxo, 'datasets', 'easy', 'sample',
                    'easy_sample_tTA2_p200_bg100_fg200_rep2000.anomaly'))
ex_path <- read.csv(file.path(vspuxo, 'datasets', 'easy', 'sample',
                    'easy_sample_tTA2_p200_bg100_fg200_rep2000.cog'))
...
<<polyfit1, warning = FALSE, fig.height = 8, fig.pos = 'p', fig.cap = ''>>=
...
par(mfrow = c(4, 3), mar = c(1, 0, 1.2, 2), las = 2, cex = 1)
plot(predict(ex_fit2, window = site_window), main = '2nd Degree')
plot(predict(ex_fit3, window = site_window), main = '3rd Degree')
plot(predict(ex_fit4, window = site_window), main = '4th Degree')
plot(predict(ex_fit5, window = site_window), main = '5th Degree')
plot(predict(ex_fit6, window = site_window), main = '6th Degree')
plot(predict(ex_fit7, window = site_window), main = '7th Degree')
plot(predict(ex_fit8, window = site_window), main = '8th Degree')
plot(predict(ex_fit9, window = site_window), main = '9th Degree')
plot(predict(ex_fit10, window = site_window), main = '10th Degree')
plot(predict(ex_fit11, window = site_window), main = '11th Degree')
plot(predict(ex_fit12, window = site_window), main = '12th Degree')
# TODO: Kernel density plot.
@
