...
set.seed(c(83534, 251, -18691, 283, -2526, 782), kind = 'L\'Ecuyer-CMRG')
@
...
\Sexpr{sprintf('%.1f', area(sitewindow) / 43560)} acre quadrilateral region
...
<<truth, cache = TRUE, fig.width = 3.75, fig.pos = 't', fig.cap = 'True intensity at the simulated UXO site.'>>=
...
par(mar = c(0, 0, 1, 3))
plot(intense.im, main = 'True Intensity Surface', zlim = c(100, 300), las = 2)
plot(setminus.owin(owin(c(1556870, 1564495), c(535421, 541130)),
                   sitewindow), col = 'white', add = TRUE)
mtext('Anomalies per Acre', 4, line = 2)
@

I simulate data collection along parallel straight-line transects spaced 390
feet apart using a detector with a footprint six feet wide. The location of
the first transect was selected randomly, and all anomalies within three feet
of a transect are observed~(Figure~\ref{fig:sim2000obs}).

<<sim2000obs>>=
# Load the sample data used in my writing project.
ex_anom <- read.anomaly(file.path(vspuxo, 'datasets', 'easy', 'sample',
                        'easy_sample_tTA2_p100_bg100_fg200_rep2000.anomaly'))
ex_path <- read.anomaly(file.path(vspuxo, 'datasets', 'easy', 'sample',
                        'easy_sample_tTA2_p100_bg100_fg200_rep2000.cog'))

par(mar = c(0, 0, 1, 0))
plot(sitewindow, main = 'Observed Geomagnetic Anomalies')
points(y ~ x, data = ex_anom, pch = '.')
@
